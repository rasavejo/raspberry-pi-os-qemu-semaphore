...
void fut_p(unsigned long page, unsigned long fut) {
    ...
    if (count == 1) call_sys_write("FUT = 1\n");
    else call_sys_write("FUT = 0\n");
...
void fut_v(unsigned long page, unsigned long fut) {
  fut_vasm(fut, page);
  if (fut_count(page,fut) == 1) call_sys_write("FUT v = 1\n");
  else call_sys_write("FUT v = 0\n");
  return;
}

int fut_count(unsigned long page,unsigned long fut) {
    return FUT(page,fut) >> 16;
}